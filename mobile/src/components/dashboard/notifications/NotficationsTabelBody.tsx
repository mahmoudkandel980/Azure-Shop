...
import ConfirmBtn from "./manipulation/ConfirmBtn";
import DeleteBtn from "./manipulation/DeleteBtn";
...
const NotficationsTabelBody = () => {
    const { baseTextColor } = useContext(ThemeContext);
    const navigation = useNavigation<NavigationProp<any>>();
    const params = useRoute<any>().params;
    const page = params ? +params?.page! : 1;

    const { usersWantToBeSellersData } = useSelector(
        (state: RootState) => state.dashbordUsers
    );
    const { ITEMS_PER_PAGE, users } = usersWantToBeSellersData!;
...
export default NotficationsTabelBody;
